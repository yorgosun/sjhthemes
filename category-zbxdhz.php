...
//图片的尺寸要在common.css里调整，下面img暂时先写样式
    ...
    ?>
		        ...
		        <p>地点：<?php echo get_post_meta(get_the_id(), '地点', true) ?></p>
		         <p>
		        <a href="<?php echo get_post_meta(get_the_id(), '周报', true) ?>"><button  type="button">周报</button></a>
		        <a href="<?php echo get_post_meta(get_the_id(), '心得', true) ?>"><button  type="button">心得</button></a>
		        <a href="<?php echo get_post_meta(get_the_id(), '照片', true) ?>"><button  type="button">照片</button></a>
		        </p>
				</li>
		<?php
endwhile;
